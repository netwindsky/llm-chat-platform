...
```bash
curl -X POST "http://localhost:38520/api/v1/models/qwen3.5-35b-a3b-thinking/load" \
...
const client = new OpenAI({
  baseURL: 'http://localhost:38520/v1',
  apiKey: 'sk-local'
});
...
| OpenAI Base URL | `http://localhost:38520/v1` |
  ...
  "models": [
    {
      ...
      "provider": "openai",
      "model": "qwen3.5-35b-a3b-thinking",
      "apiBase": "http://localhost:38520/v1",
      "apiKey": "sk-local"
    },
    {
...
3. 添加自定义提供商：
   - **名称**: LLM Platform
   - **类型**: OpenAI 兼容
   - **Base URL**: `http://localhost:38520/v1`
   - **API Key**: `sk-local`
...
2. 添加 OpenAI 兼容提供商：
   - **名称**: LLM Platform
   - **API URL**: `http://localhost:38520/v1`
   - **API Key**: `sk-local`
...
docker run -d \
  ...
  -e BASE_URL=http://localhost:38520/v1 \
...
```bash
# 检查服务状态
curl http://localhost:38520/health

# 检查端口占用
netstat -an | findstr 38520  # Windows
lsof -i :38520               # Linux/Mac
```
